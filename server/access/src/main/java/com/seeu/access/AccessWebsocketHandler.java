...
public class AccessWebsocketHandler extends WebsocketHandler {
    ...
    public void recordHttpUsefulHeaders(ChannelHandlerContext ctx, HttpHeaders httpHeaders) {
        String realIp = httpHeaders.get("X-Real-IP");
        log.info("the connection recordHttpUsefulHeaders httpHeaders is {}; ctx is {}",
                httpHeaders,
                ctx.channel().remoteAddress().toString());
        if (null == realIp) {
            log.info("the connection is not by transfer! remote: {}",
                    ctx.channel().remoteAddress().toString());
            return;
        }
    ...
    public void handleJsonMessage(ChannelHandlerContext ctx, String msg) {
        try {
            ...
            log.error("catch an exception: msg：{}, cause: {}, exception:", e.getMessage(),
                    e.getCause(), e);
    ...
    public void handleProtobufMessage(ChannelHandlerContext ctx, MessageLite msg) {
        try {
            ...
            //消息处理
            log.debug("handleProtobufMessage receive msg: " + JsonFormat.printToString(wsMsg));
        } catch (Exception e) {
            log.error("catch an exception: msg：{}, cause: {}, exception:", e.getMessage(),
                    e.getCause(), e);
        }
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        log.info("channelInactive, ctx:" + ctx.toString() + ", remoteAddress:" + ctx.channel()
