...
public class UserConnectionMgr implements Serializable {
	private static final Logger logger = LoggerFactory.getLogger(UserConnectionMgr.class);
...
//    }
	...
	public AccessUserInfo addUserInfo(long uid, String devId, ChannelHandlerContext ctx) {
		...
		logger.debug("add user info:" + info.toString());
	...
	public void sendMsgByUid(long uid, String msgid, Object msg) {
		...
		if (null == info) {
			logger.error("send msg to user uid: " + uid + " but cannot get user info!");
			return;
		}
	...
	public void sendMsgByDevId(String devId, String msgid, Object msg) {
		...
		if (null == info) {
			logger.error("send msg to user devId: " + devId + " but cannot get user info!");
			return;
		}
