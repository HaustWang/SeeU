...
public class RpcClientManger {
        ...
        for (Map<Integer, RpcBaseClient> clientMap : rpcClientMap.values()) {
            for (RpcBaseClient client : clientMap.values()) {
                try {
                    ...
                    log.error("broadcast to {}:{} catch an exception: ", client.getHost(),
                            client.getPort(), e);
                }
            }
        }
    ...
    public RpcMsg.response request(ServerType serverType, int serverId, String method,
        ...
        if (null == client) {
            ...
            if (null == infoResp) {
                log.error("can't get any server, serverType: {}, serverId: {}", serverType,
                        serverId);
                return null;
            }

            if (infoResp.getSvrId() != serverId) {
                log.warn("get server {} and id {} but return id is {}", serverType, serverId,
                        infoResp.getSvrId());
            }
        ...
        try {
            return clientHandler.write(request, Objects.requireNonNull(client));
        } catch (Exception e) {
            log.error(
                    "broadcast to servertype: {}, serverid: {}, address: {}:{} catch an exception: ",
                    serverType, serverId, client != null ? client.getHost() : null,
                    client != null ? client.getPort() : 0, e);
            return null;
        }
    }
